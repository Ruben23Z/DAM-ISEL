...
/**
 * WeatherViewModel: Componente da camada de Apresentação fundamentado no padrão MVVM. Atua como
 * mediador entre a fonte de dados (WeatherRepo) e a Interface de Utilizador (UI). Gere o estado
 * reativo da aplicação através de instâncias de LiveData.
 */
class WeatherViewModel(application: Application) : AndroidViewModel(application) {
  ...
  val liveWeatherData: LiveData<WeatherData>
    get() = weatherData
  ...
  /**
   * Processa a entrada de texto do utilizador para extrair latitude e longitude. Suporta diversos
   * formatos de entrada (espaço, vírgula ou ponto-e-vírgula como separadores).
   */
  ...
  /**
   * Inicializa o mapa de mapeamento meteorológico a partir dos recursos do sistema (strings.xml /
   * arrays.xml).
   */
  ...
  /**
   * Implementação da lógica de tratamento e formatação de dados meteorológicos. Este método
   * transforma dados brutos da API em representações legíveis para o utilizador final.
   */
    ...
    if (info == null) {
      ...
      val resID =
        getApplication<Application>()
          .resources
          .getIdentifier(name, "drawable", getApplication<Application>().packageName)
...
//
/// **
// * BindingAdapter: Utilitário para a orquestração de propriedades costumizadas no paradigma Data
// Binding.
// */
