...
class WeatherViewModel(application: Application) : AndroidViewModel(application) {
    ...
    //permite que quando o valor das variaveis altera o Xml atualiza se sozinho
    ...
    private fun processWeatherdata(request: WeatherData) {
        ...
        // 1. Processamento e formatação da temporização local da localização (ISO-8601)
        val fullTime = current.time ?: ""
        val parts = fullTime.split("T")
        val tempoValor = if (parts.size >= 2) parts[1] else "--:--"
